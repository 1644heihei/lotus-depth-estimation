...
def parse_args():
    ...
    p.add_argument("--max_images", type=int, default=0, help="0 = all rgb_cam_*.png under rgb_dir")
...
def iter_rgb_images(rgb_dir: Path, max_images: int = 0) -> List[Path]:
    files = sorted(rgb_dir.rglob("rgb_cam_*.png"))
    if max_images > 0:
...
def main():
    ...
    paths = iter_rgb_images(rgb_dir, max_images=args.max_images)
    if not paths:
        raise FileNotFoundError(f"No rgb_cam_*.png found under {rgb_dir}")
