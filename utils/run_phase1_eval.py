...
Usage:
  python utils/run_phase1_eval.py \
      --output_dir output/phase1_roi_fusion_bbox \
      [--detector yolo] \
      [--max_images 20]
"""
...
from utils.object_image import make_object_image
...
def parse_args():
    ...
    p.add_argument(
        ...
        default="yolo",
        choices=["yolo", "mask"],
        help="yolo: bbox detect and crop; mask: legacy precomputed seg masks.",
    )
    p.add_argument("--mask_dir", type=str, default=None, help="Required when --detector mask.")
    p.add_argument("--yolo_model", type=str, default="yolov8n.pt")
    p.add_argument("--yolo_score_thr", type=float, default=0.25)
    p.add_argument("--yolo_labels", type=str, default="", help="Comma-separated COCO names; empty = all.")
    ...
    p.add_argument(
        ...
        default="crop",
        choices=["crop", "masked"],
        help="crop: bbox+context; masked: zero background inside bbox.",
    )
    return p.parse_args()
...
def main():
    ...
    if args.detector == "mask" and not args.mask_dir:
        raise ValueError("--mask_dir is required when --detector mask")
    ...
    for idx in tqdm(range(num_samples), desc="Phase1 eval"):
        ...
        if args.detector == "yolo":
            ...
            roi_mask = bboxes_to_roi_mask(h, w, raw_boxes)
            for box in raw_boxes:
                x1, y1, x2, y2 = expand_box(box, (h, w), expand_ratio=args.roi_expand_ratio)
                obj_rgb = rgb_np[y1:y2, x1:x2] if args.object_image_mode == "crop" else make_object_image(
                    rgb_np, roi_mask, (x1, y1, x2, y2), mode=args.object_image_mode
                )
                if obj_rgb.size == 0:
                    continue
                crop_preds.append(((x1, y1, x2, y2), predict_disparity(pipe, obj_rgb, args, generator)))
        else:
        ...
        fusion_mask = roi_mask if args.detector == "mask" else None
