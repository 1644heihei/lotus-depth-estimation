...
def parse_args():
    ...
    parser.add_argument("--roi_expand_ratio", type=float, default=0.12)
...
def main():
    ...
    for image_path in tqdm(images, desc="ROI fusion inference"):
        ...
        roi_depth_canvas = global_depth.copy()
        ...
        for box in boxes:
            ...
            crop_depth = np.array(
                Image.fromarray(crop_depth.astype(np.float32), mode="F").resize((x2 - x1, y2 - y1), resample=Image.BILINEAR)
            )
            roi_depth_canvas[y1:y2, x1:x2] = crop_depth
            effective_mask[y1:y2, x1:x2] = np.maximum(effective_mask[y1:y2, x1:x2], roi_mask[y1:y2, x1:x2])

        soft = make_soft_weight(effective_mask, blur_ksize=args.blend_blur_ksize)
        wmap = np.clip(soft * args.fusion_weight, 0.0, 1.0)
        fused_depth = wmap * roi_depth_canvas + (1.0 - wmap) * global_depth
